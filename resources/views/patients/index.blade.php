<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Find Patient</h2>
            ...
            <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-5 mb-6">
                <form method="GET" action="{{ route('patients.index') }}">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Post Code</label>
                            <input type="text" name="post_code" value="{{ request('post_code') }}"
                                class="w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Sex / Gender</label>
                            <select name="sex_gender" class="w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Patient Type</label>
                            <select name="patient_type" class="w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">— Any —</option>
                        ...
                        <div class="flex items-end gap-3">
                            <label class="flex items-center gap-1.5 text-sm text-gray-700 pb-1">
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="bg-gray-50 border-b border-gray-200">
                                    ...
                                    <th class="text-left px-4 py-2 font-medium text-gray-600">DOB</th>
                                    <th class="text-left px-4 py-2 font-medium text-gray-600">Post Code</th>
                                    <th class="text-left px-4 py-2 font-medium text-gray-600">Type</th>
                                    <th class="text-left px-4 py-2 font-medium text-gray-600">Flags</th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($results as $patient)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-2 text-gray-500 font-mono">{{ $patient->id }}</td>
                                        ...
                                        <td class="px-4 py-2 text-gray-600">{{ $patient->date_of_birth->format('d/m/Y') }}</td>
                                        <td class="px-4 py-2 text-gray-600">{{ $patient->post_code }}</td>
                                        <td class="px-4 py-2 text-gray-600">{{ $patient->patient_type?->label() }}</td>
                                        <td class="px-4 py-2">
