...
- **RxMeasurement value object** — the Transpose Rx button applies an optical calculation to the SPH/CYL/Axis block. This belongs in a value object, not a controller
- **CachedPatientRepository** — the patient search screen with 9 filters is the natural first candidate for caching; the interface binding makes this a one-line swap
- **Normalised medications pivot table** — replace the JSON column with a proper `medications` + `examination_medications` schema
- **PDF report generation** — DomPDF + a Blade report template + `signed_examinations` storage
- **Recalls / follow-up scheduling** — a RecallStrategy that calculates the next appointment date based on `retest_after` and patient type